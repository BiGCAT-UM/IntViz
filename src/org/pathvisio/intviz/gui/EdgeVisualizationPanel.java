...
/**
 * Main component of the {@link VisualizationDialog}. Has capabilities to add /
 * remove / rename visualizations, and has a subpanel for all available
...
public class EdgeVisualizationPanel extends JPanel implements ActionListener {
	...
	public EdgeVisualizationPanel(VisualizationManager visMgr) {
		...
		visButton = new DropDownButton(new ImageIcon(
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (ACTION_NEW.equals(action)) {
			...
			visMgr.setActiveVisualization((Visualization) visCombo
					.getSelectedItem());
			refresh();
		}
	...
	private void refresh() {
		...
		if (visMgr != null) {
			...
			visCombo.setModel(new DefaultComboBoxModel(visMgr
					.getVisualizations().toArray()));
